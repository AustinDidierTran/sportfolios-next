...
export default function GraphLinear(props) {
  ...
  return (
    <div className={styles.root}>
            ...
            <div style={{ minHeight: 53.75 }}>
              ...
              {value - initialValue !== 0 && (
                <div style={{ color: value - initialValue > 0 ? 'green' : 'red', fontSize: 18 }}>
                  {value - initialValue > 0 ? '+' : ''}{formatData(value - initialValue)} {value - initialValue > 1 ? t(lines[0]?.name) : t(lines[0]?.nameSingular)} ({percentage} %)
                </div>
              )}
            </div>
          )}
      ...
      { !data.length && (
        <div>{t('choose.choose_later_date')}</div>
      )}
    </div>
  );
}
