...
export default function Ranking() {
  ...
  categories.map((categorie) => categorie.teams.sort((a, b) => a.position - b.position));
          ...
          {categorie.teams.map((team, teamIndex) => (
            <List className={styles.list} key={teamIndex}>
              <ListItem className={styles.listItem}>
                <Typography color="primary" variant="h5" className={styles.position}>
                  {team.position}
                </Typography>
                <Avatar className={styles.avatar} initials={team.initials} photoUrl={team.photoUrl} size="sm" />
